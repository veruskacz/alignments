...
def linkset_geo_metadata(specs, display=False):
    ...
    # specs[St.linkset] = "{}{}".format(Ns.linkset, specs[St.linkset_name])
    specs[St.singleton] = "{}{}".format(Ns.singletons, specs[St.linkset_name])
    specs[St.link] = "{}{}{}".format(Ns.alivocab, "nearbyGeoSim", specs[St.sameAsCount])
    specs[St.assertion_method] = "{}{}".format(Ns.method, specs[St.linkset_name])
    specs[St.justification] = "{}{}".format(Ns.justification, specs[St.linkset_name])
    specs[St.link_comment] = "The predicate <{}> used in this linkset is a property that reflects an entity " \
                             "linking approach based on the <{}{}> mechanism.". \
...
def lens_refine_geo_metadata(specs, display=False):
    ...
    specs[St.justification] = "{}{}".format(Ns.justification, specs[St.lens_name])
    specs[St.link_comment] = "The predicate <{}> used in this linkset is a property that reflects an entity " \
                             "linking approach based on the <{}{}> mechanism.". \
        format(specs[St.link], Ns.mechanism, specs[St.mechanism])
...
def intersection_meta(specs):
    ...
    {{
        ...
        <{0}>
            a                                   bdb:Lens ;
            rdfs:label                          "{9}" ;
            alivocab:operator                   lensOp:intersection ;{6}
            void:triples                        {7} ;
            alivocab:singletonGraph             specific:{9} ;
            bdb:assertionMethod                 <{8}{9}> .
        ### ASSERTION METHOD"
        <{8}{9}>
            alivocab:sparql           \"\"\"{10}\"\"\" .
    }}""".format(
        # 0             1        2            3        4       5
        specs[St.lens], Ns.rdfs, Ns.alivocab, Ns.void, Ns.bdb, Ns.lensOp,
        # 6                            7                  8          9
        specs[St.lens_target_triples], specs[St.triples], Ns.method, specs[St.lens_name],
        # 10                    11       11
        specs[St.insert_query], Ns.singletons)
